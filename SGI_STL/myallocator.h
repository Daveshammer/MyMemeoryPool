...
__malloc_alloc_template<__inst>::_S_oom_malloc(size_t __n)
{
	...
	for (;;) {
		...
		if (nullptr == __my_malloc_handler) { throw std::bad_alloc(); } //没有预置回调
		(*__my_malloc_handler)();
		__result = malloc(__n);  //释放一些资源后，再进行malloc
...
template <typename T>
size_t my_allocator<T>::_S_heap_size = nullptr;
...
T* my_allocator<T>::allocate(size_t __n)
{
	...
	if (__n > (size_t)_MAX_BYTES) {
		...
		_Obj* volatile* __my_free_list // 指向数组中的8 16 24 ...
		...
		_Obj* __result = *__my_free_list; // 获取指向数组中的值
		if (__result == 0) // 数组中该大小的块为空
			__ret = _S_refill(_S_round_up(__n)); // 指向新分配内存池的首节点
		else {
			*__my_free_list = __result->_M_free_list_link; // 存放下一个空闲节点的地址
			__ret = __result;
		}
	}
...
void my_allocator<T>::deallocate(void* __p, size_t __n)		//  因为vector容器传来的是字节大小
{
	...
	else
	{
		...
		__q->_M_free_list_link = *__my_free_list; // 头插法，将q插入到空闲链表中
		*__my_free_list = __q;
...
void* my_allocator<T>::_S_refill(size_t __n)  //  n是一个chunk块的大小
{
	...
	*__my_free_list = __next_obj = (_Obj*)(__chunk + __n);    //  __n：一个内存块的大小（因为第一个内存块要分配出去）
	for (__i = 1; ; __i++) {  //  仅初始化__nobjs个块成静态链表，备用块不处理
		__current_obj = __next_obj;
		__next_obj = (_Obj*)((char*)__next_obj + __n);          //   维护内存块间的连接  char* 因此+__n是偏移n个bytes +n是为了一次跑一个chunk块
		if (__nobjs - 1 == __i) { //   最后一个chunk节点
			__current_obj->_M_free_list_link = 0; //   表示最后一个chunk节点后面没有空闲块
			break;
		}
		else {
